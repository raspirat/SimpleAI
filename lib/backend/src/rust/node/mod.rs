...
use crate::{errors::*, info::*, stdpaths::*, util::default_hash::DefaultHash};
...
impl TryFrom<(Info, Nodes)> for Node {
    ...
    fn try_from(value: (Info, Nodes)) -> Result<Self, Self::Error> {
        ...
        Err(Error::NodeConversionError(Args::new(
            ErrorTypes::Default,
...
pub fn to_infos(nodes: Nodes) -> Infos {
    nodes
        .iter()
        .map(|node: &Node| node.node_decl.info.clone())
        .collect::<Infos>()
}
